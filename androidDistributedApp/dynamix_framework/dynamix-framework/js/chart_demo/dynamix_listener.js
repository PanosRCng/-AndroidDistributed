...
if (typeof DynamixListener === "undefined") {
	...
	DynamixListener.onSecurityAuthorizationGranted = function() {
		...
		Dynamix.addContextSupport('org.ambientdynamix.contextplugins.zephyrhxm');
	};

	/*
	 * Called if the user revokes the web client's security authorization. This
	 * event is followed by 'DynamixListener.onContextSupportRemoved' for
	 * any existing context support, and then
	 * 'DynamixListener.onSessionClosed'. possibly
	 */
	 ...
	 * this event the web client may add context support, perform
	 * context requests, etc.
	 */
	 ...
	 * 'Dynamix.addContextSupport' or
	 * 'Dynamix.addConfiguredContextSupport'. See
	 * 'Dynamix.ContextSupportInfo' in dynamix.js for details on the
	 * properties of the subInfo object.
	 */
	DynamixListener.onContextSupportAdded = function(subInfo) {
		console.log("onContextSupportAdded: " + subInfo.contextType
				+ " using plug-in " + subInfo.plugin.pluginId + " "
				+ subInfo.plugin.version);
	};

	/*
	 * Called after context support has been removed. Raised in response
	 * to 'Dynamix.removeContextSupportForContextType',
	 * 'Dynamix.removeContextSupportForSupportId',
	 * 'Dynamix.removeContextSupports', or any time Dynamix removes context
	 * support (e.g., when shutting down). See
	 * 'Dynamix.ContextSupportInfo' in dynamix.js for details on the
	 * properties of the subInfo object.
	 */
	DynamixListener.onContextSupportRemoved = function(subInfo) {
		console.log("onContextSupportRemoved: " + subInfo.contextType
				+ " using plug-in " + subInfo.plugin.pluginId + " "
				+ subInfo.plugin.version);
	};

	/*
	 * Called if the requested context support cannot be provided for the
	 * specified context type. Raised after a failure of
	 * 'Dynamix.addContextSupport' or
	 * 'Dynamix.addConfiguredContextSupport'.
	 */
	 ...
	 * context support. Raised in response to
	 * 'Dynamix.addContextSupport' or
	 * 'Dynamix.addConfiguredContextSupport'. See
	...
	/*
	 * Called when a context request fails. Raised in response to a
	 * failure of 'Dynamix.contextRequest' or
	 * 'Dynamix.configuredContextRequest'.
	 */
	...
	DynamixListener.heart_rate = 0;
	/*
	 * Called when requested context information has been discovered for a
	 * particular context type. For auto plug-ins, this event is
	 * raised automatically when new context information is available. For
	 * reactive plug-ins, this event is raised after a successful call to
	...
	DynamixListener.onContextEvent = function(event) {
		...
		console.log("onContextEvent heart_rate: " + event.heart_rate);
		console.log("onContextEvent Speed: " + event.speed);
		DynamixListener.heart_rate = parseInt(event.heart_rate);
	};
